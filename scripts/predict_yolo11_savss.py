...
def main() -> None:
    ...
    ssm_mode = str(saved_args.get("ssm_mode", "dynamic"))
    savss_stages = str(saved_args.get("savss_stages", "enc3,up3"))
    savss_n = int(saved_args.get("savss_n", 1))

    model = YOLO11SAVSSSeg(base_ch=base_ch, deep_supervision=False, scan_impl=scan_impl, ssm_mode=ssm_mode, savss_stages=savss_stages, savss_n=savss_n).to(device)
